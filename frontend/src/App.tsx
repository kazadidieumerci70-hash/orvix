...
function GuestChatView({ onRequireAuth }: { onRequireAuth: () => void }) {
  ...
  return <section className="chat-view guest-chat-view"><header className="guest-topbar"><div className="brand"><img className="brand-logo" src="/orvix-logo-transparent.png" alt="Logo Orvix" /><span>ORVIX</span></div><div className="guest-topbar-actions"><button type="button" className="guest-menu-button" onClick={() => setMenuOpen((open) => !open)} aria-label="Ouvrir le menu" aria-expanded={menuOpen}><Menu /></button><label className="top-document-selector"><FileText size={17} /><select aria-label="Mode de réponse" defaultValue=""><option value="">Question libre</option></select></label><button type="button" className="guest-profile-button" onClick={onRequireAuth} aria-label="Se connecter"><User /></button></div></header>{menuOpen && <><button className="guest-menu-backdrop" onClick={() => setMenuOpen(false)} aria-label="Fermer le menu" /><aside className="guest-menu-panel"><div className="brand"><img className="brand-logo" src="/orvix-logo-transparent.png" alt="Logo Orvix" /><span>ORVIX</span></div>{["Chat", "Révision", "Quiz", "Supports"].map((item) => <button key={item} type="button" className={item === "Chat" ? "active" : ""} onClick={() => item !== "Chat" && onRequireAuth()}>{item}</button>)}<button type="button" className="guest-menu-login" onClick={onRequireAuth}>Se connecter</button></aside></>}<div className="messages"><div className="chat-empty"><div className="brand chat-empty-brand"><img className="brand-logo chat-empty-logo" src="/orvix-logo-transparent.png" alt="Logo Orvix" /></div><div className="orvix-introduction regular-welcome"><h1>Que veux-tu comprendre aujourd’hui ?</h1></div></div></div><form className="composer guest-composer" onSubmit={submit}><div className="composer-tools"><button className="composer-support" type="button" aria-label="Ajouter une pièce jointe"><Plus size={18} /></button></div><div className="composer-input-stack"><textarea rows={1} value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Écrivez votre message…" aria-label="Écrivez votre message…" /></div><button disabled={!message.trim()} aria-label="Envoyer"><Send size={21} fill="currentColor" /></button></form></section>;
}
