  ...
  return <section className={`chat-view ${messages.length || loading ? "has-messages" : "empty-chat"}`}>
    ...
    <form ref={composerRef} className={`composer ${composerExpanded ? "has-message" : ""} ${message ? "is-typing" : ""} ${mobileComposer ? "mobile-flat-input" : ""}`} onSubmit={submit}>
      ...
      <div className="composer-input-stack">
        <textarea className={composerOverflowing ? "is-overflowing" : ""} ref={textareaRef} rows={1} value={message} onChange={(event) => { const value = event.target.value; setMessage(value); setComposerExpanded(value.includes("\n") || value.length > 72); const textarea = event.currentTarget; textarea.style.setProperty("height", "auto", "important"); const maxHeight = 120; const nextHeight = Math.min(Math.max(textarea.scrollHeight, 24), maxHeight); textarea.style.setProperty("height", `${nextHeight}px`, "important"); textarea.style.setProperty("overflow-y", textarea.scrollHeight > maxHeight ? "auto" : "hidden", "important"); setComposerOverflowing(textarea.scrollHeight > maxHeight + 1); }} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder={chatText.placeholder} aria-label={chatText.placeholder} />
      </div>
